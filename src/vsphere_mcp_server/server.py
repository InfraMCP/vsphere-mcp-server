...
def _handle_error(e: Exception, operation: str) -> str:
    ...
    if "Authentication failed" in error_msg:
        return f"Authentication failed for {operation}. Try clearing credentials with vsphere_clear_credentials."
...
def get_vm_details(hostname: str, vm_id: str) -> str:
    ...
    try:
        ...
        result += f"Hardware Version: {vm.get('hardware', {}).get('version', 'Unknown')}\n"
...
def power_on_vm(hostname: str, vm_id: str) -> str:
    ...
    client = VSphereClient(hostname)
    try:
        response = client.post(f"vcenter/vm/{vm_id}/power/start")
        return f"Power on initiated for VM {vm_id}"
...
def power_off_vm(hostname: str, vm_id: str) -> str:
    ...
    client = VSphereClient(hostname)
    try:
        response = client.post(f"vcenter/vm/{vm_id}/power/stop")
...
def list_folders(hostname: str, folder_type: str = "VIRTUAL_MACHINE") -> str:
    ...
    try:
        ...
        for folder in folders:
            result += f"• {folder.get('name', 'Unknown')} (ID: {folder.get('folder')})\n"
